...
terrainr is built to play nicely with functions from the sf and raster packages.
In order to get our simulated points into the right format, we need to use the 
`st_as_sf` function from the `sf` package:
...
addition to other raster data from the National Map. These programs each have
...
splitting your request into tiles and downloading tiles separately. To make
