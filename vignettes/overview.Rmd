...
progress bars, if the user requests them via the [progressr](https://cran.r-project.org/web/packages/progressr/vignettes/progressr-intro.html) 
...
```{r echo = FALSE}
output_files <- list(
  `3DEPElevation` = "/tmp/RtmpzuCNgk/file40de6193a909a_3DEPElevation_1_1.tiff",
  USGSNAIPPlus = "/tmp/RtmpzuCNgk/file40de6193a909a_USGSNAIPPlus_1_1.png"
)
output_files
```
...
```{r eval=FALSE}
raster::plot(raster::raster(output_files[[2]]))
```
...
Now, a weird quirk of this API is that orthoimages are not 
georeferenced when you first download them. However, the image tiles produced 
by terrainr do precisely align with the DEM files that we downloaded for the 
same tile. A second weird quirk is that the USGS does not necessarily respect 
the bounding boxes that we ask for, and will download tiles with overlapping
edges. 

We can fix both of these problems at the same time using `merge_rasters`,
which will take the vectors of output files from `get_tiles` and merge them 
while georeferencing the output orthoimage:

```{r eval = FALSE}
output_merged <- merge_rasters(input_rasters = output_files[1],
                               output_raster = tempfile(fileext = ".tif"),
                               input_images = output_files[2],
...
```{r eval = FALSE}
side_length <- vapply(output_merged, 
...
mapply(function(x, y, z) {
  ...
  output_merged, # our list pointing to the merged images
...
```{r echo = FALSE}
knitr::include_graphics("ebert_unity.png")
```
