...
raster_to_raw_tiles <- function(input_file, output_prefix, side_length = 4097) {
  ...
  for (i in seq(0, input_raster@nrows, side_length)) {
    for (j in seq(0, input_raster@ncols, side_length)) {
      gdalUtils::gdal_translate(input_file, temptiffs[[counter]],
                                srcwin = paste0(i, ", ", j, ", side_length, side_length")
      )
